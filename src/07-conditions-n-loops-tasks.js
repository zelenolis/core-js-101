...
function doRectanglesOverlap(rect1, rect2) {
  ...
  if (rect2.top + rect2.height > rect1.top && rect2.top + rect2.height < rect1.top + rect1.height
    && rect2.left > rect1.left && rect2.left < rect1.left + rect1.width) {
    return true;
  }
  if (rect2.top > rect1.top && rect2.top < rect1.top + rect1.height
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  if (a < b) { answer += `${a}, ${b}`; } else { answer += `${b}, ${a}`; }
  if (isEndIncluded) {
    answer += ']';
  } else { answer += ')'; }
  return answer;
...
function isCreditCardNumber(ccn) {
  ...
  const newArr = arr.map((digit, index) => {
    if (index % 2 === 0) {
      return +digit;
    }
...
function isBracketsBalanced(str) {
  let strr = str;
  let square = strr.indexOf('[]');
  let round = strr.indexOf('()');
  let curve = strr.indexOf('{}');
  let angle = strr.indexOf('<>');
  do {
    if (strr === '') { return true; }
    square = strr.indexOf('[]');
    round = strr.indexOf('()');
    curve = strr.indexOf('{}');
    angle = strr.indexOf('<>');
    if (square > -1) { strr = strr.replace('[]', ''); }
    if (round > -1) { strr = strr.replace('()', ''); }
    if (curve > -1) { strr = strr.replace('{}', ''); }
    if (angle > -1) { strr = strr.replace('<>', ''); }
  } while (square > -1 || round > -1 || curve > -1 || angle > -1);
  return false;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 1; i < pathes.length; i += 1) {
    for (let j = 0; j < pathes[i].length; j += 1) {
      if (p.indexOf(pathes[i].slice(0, j)) < 0) {
        p = pathes[i].slice(0, j - 1); break;
      }
    }
  }
